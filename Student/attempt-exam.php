<?php
    ...
    while ($row = mysqli_fetch_assoc($questions_run)) {
        $questions[] = [
            ...
            "matching" => json_decode($row['Options'], true),
            "image" => $row['Diagram_Image'] !== null ? (function($blob){
                $finfo = finfo_open(FILEINFO_MIME_TYPE);
                $mime = finfo_buffer($finfo, $blob) ?: 'image/png';
                finfo_close($finfo);
                return "data:$mime;base64," . base64_encode($blob);
            })($row['Diagram_Image']) : null,
            "marks" => $row['Marks']
        ];
    }
...
<head>
    ...
    <title>Student - Attempt Exam</title>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .exam-sidebar h6 {
                display: none; /* remove title on mobile */
            }
            ...
            /* Circle buttons smaller for mobile */
            .question-btn {
            ...
            main {
                ...
                margin-bottom: 80px; /* space above bottom bar */
            }
            .navbar{
                font-size: 15px;
            }
    ...
    <script>
        ...
        function updateStatusColors() {
            document.querySelectorAll(".question-btn").forEach((btn, i) => {
                const q = questions[i];  // ✅ FIXED — now q is defined
                let isAnswered = false;
                if (q.type == 2) {
                    isAnswered = answers[i] &&
                                Object.keys(answers[i]).length === Object.keys(q.matching).length;
                } else {
                    isAnswered = !!answers[i];
                }
                ...
                btn.className = "question-btn " + (
                    ...
                    isAnswered ? "answered" :
                    visited.has(i) ? "not-answered" :
                    "not-visited"
                );
            });
        }

        // Universal question loader (MCQ + Fill-up)
        function loadQuestion(i) {
            ...
            else if (q.type == 2 && q.matching) {
                const leftItems = Object.keys(q.matching);
                const rightItems = Object.values(q.matching);

                // Shuffle right side
                const shuffledRight = [...rightItems].sort(() => Math.random() - 0.5);

                html += `<div class="row">
                    <div class="col-md-6"><strong>Left Side</strong></div>
                    <div class="col-md-6"><strong>Right Side</strong></div>
                </div><hr>`;

                leftItems.forEach((left, idx) => {
                    const saved = answers[i]?.[left] || "";

                    html += `
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="p-2 border rounded bg-light">${left}</div>
                            </div>
                            <div class="col-md-6">
                                <select class="form-select" onchange="selectMatch(${i}, '${q.qid}', '${left}', this.value)">
                ...
                html += `
                    <div class="mb-3">
                        <textarea class="form-control" rows="4" placeholder="Type your answer here"
                            oninput="selectOption(${i}, '${q.qid}', this.value)">${answers[i] || ''}</textarea>
                    </div>
                `;
            }
            else if (q.type == 9) {
                // Diagram-based Question UI
                html += `
                    <div class="mb-4 text-center">
                        <img src="${q.image}" alt="Diagram" 
                            style="max-width: 100%; height: auto; border-radius: 10px; border:1px solid #ccc;">
                    </div>
                    <div class="mb-3">
                        <textarea class="form-control" rows="4" placeholder="Type your answer here"
                            oninput="selectOption(${i}, '${q.qid}', this.value)">${answers[i] || ''}</textarea>
        ...
        function selectMatch(i, qid, leftKey, rightVal) {

            // Maintain structured answers
            if (!answers[i]) answers[i] = {};
            answers[i][leftKey] = rightVal;

            updateStatusColors();

            // Create hidden input name="answers[qid][left]"
            let name = `answers[${qid}][${leftKey}]`;
